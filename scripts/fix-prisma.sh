#!/usr/bin/env bash
set -euo pipefail

echo "🔧 Fixing Prisma schema issues..."

# Navigate to the db package
cd packages/db

# Clear Prisma cache
echo "Clearing Prisma cache..."
rm -rf node_modules/.prisma
rm -rf prisma/migrations

# Create a simple working schema first
cat > prisma/schema.prisma << 'EOF'
generator client {
  provider = "prisma-client-js"
}

datasource db {
  provider = "postgresql"
  url      = env("DATABASE_URL")
}

model User {
  id        String   @id @default(cuid())
  email     String   @unique
  name      String?
  image     String?
  createdAt DateTime @default(now())
  updatedAt DateTime @updatedAt
  accounts  Account[]
  sessions  Session[]
  projects  Project[]
}

model Account {
  id                 String  @id @default(cuid())
  userId             String
  type               String
  provider           String
  providerAccountId  String
  refresh_token      String?
  access_token       String?
  expires_at         Int?
  token_type         String?
  scope              String?
  id_token           String?
  session_state      String?
  user               User    @relation(fields: [userId], references: [id], onDelete: Cascade)
  @@unique([provider, providerAccountId])
}

model Session {
  id           String   @id @default(cuid())
  sessionToken String   @unique
  userId       String
  expires      DateTime
  user         User     @relation(fields: [userId], references: [id], onDelete: Cascade)
}

model VerificationToken {
  identifier String
  token      String   @unique
  expires    DateTime
  @@unique([identifier, token])
}

model Project {
  id        String   @id @default(cuid())
  ownerId   String
  name      String   @default("default")
  createdAt DateTime @default(now())
  user      User     @relation(fields: [ownerId], references: [id], onDelete: Cascade)
  files     File[]
  @@index([ownerId])
}

model File {
  id           String     @id @default(cuid())
  projectId    String
  ownerId      String
  originalName String
  mimeType     String
  byteSize     BigInt
  sha256Hex    String     @db.VarChar(64)
  storageKey   String
  createdAt    DateTime   @default(now())
  status       FileStatus @default(PROCESSED)
  project      Project    @relation(fields: [projectId], references: [id], onDelete: Cascade)
  assets       Asset[]
  @@index([ownerId])
  @@index([projectId])
  @@index([sha256Hex])
}

enum FileStatus { UPLOADED PROCESSED FAILED }

model Asset {
  id        String   @id @default(cuid())
  fileId    String
  ownerId   String
  kind      AssetKind
  pageIndex Int?
  width     Int?
  height    Int?
  thumbKey  String?
  createdAt DateTime @default(now())
  file      File     @relation(fields: [fileId], references: [id], onDelete: Cascade)
  hashes    Hash[]
  clusters  ClusterMember[]
  @@index([ownerId])
}

enum AssetKind { IMAGE PDF_PAGE }

model Hash {
  id        String   @id @default(cuid())
  assetId   String
  kind      HashKind
  hex64     String   @db.VarChar(16)
  bucket16  Int
  createdAt DateTime @default(now())
  asset     Asset    @relation(fields: [assetId], references: [id], onDelete: Cascade)
  @@index([assetId])
  @@index([bucket16])
  @@index([kind, bucket16])
}

enum HashKind { PHASH AHASH DHASH }

model DuplicateCluster {
  id        String   @id @default(cuid())
  ownerId   String
  createdAt DateTime @default(now())
  updatedAt DateTime @updatedAt
  members   ClusterMember[]
  @@index([ownerId])
}

model ClusterMember {
  id        String  @id @default(cuid())
  clusterId String
  assetId   String
  isKeeper  Boolean @default(false)
  cluster   DuplicateCluster @relation(fields: [clusterId], references: [id], onDelete: Cascade)
  asset     Asset            @relation(fields: [assetId], references: [id], onDelete: Cascade)
  @@unique([clusterId, assetId])
  @@index([clusterId])
}

model AuditLog {
  id        String   @id @default(cuid())
  ownerId   String
  action    String
  payload   Json
  createdAt DateTime @default(now())
  @@index([ownerId])
}
EOF

# Set environment variable for validation
export DATABASE_URL="postgresql://postgres:postgres@localhost:5432/dup"

# Validate the schema
echo "Validating schema..."
npx prisma validate

# Generate the client
echo "Generating Prisma client..."
npx prisma generate

echo "✅ Prisma schema fixed and client generated!"
echo ""
echo "Next steps:"
echo "1. Start the database: docker compose up -d postgres"
echo "2. Push the schema: pnpm db:push"
echo "3. Start the application: pnpm dev"
